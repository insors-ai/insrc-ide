...
| CFG-based flow                            | partial | TS / TSX / JS done via on-demand tree-sitter walk on the entity's `body`. Recognised nodes: branches (if/switch), loops (for/for-in/for-of/while/do-while), exception flow (try/catch/finally), terminators (return/break/continue/throw). Mermaid `flowchart TD` with branch true/false labels + loop back-edges + dotted throw arrows. 200-node cap per function; overflow falls through to the phase-1 Kuzu CALLS approximation. **Python + Go walkers pending follow-up PRs** -- per-language tree-sitter node names differ. **No indexer changes.** |
