...
src/insrc/agent/tasks/artifacts/
  ...
  kinds/
    ...
    wireframe-classifiers/                    # phase 4 (§4.1) -- per-library TS const dictionaries
...
| Item                                      | Status | Notes |
|-------------------------------------------|--------|-------|
| React introspection for wireframes        | done   | Layout-sketch fidelity (not faithful render). Three-layer JSX classifier (layout container / semantic element / unknown). v1 libraries: native HTML + MUI + Chakra + AntD + shadcn/ui + Tailwind utility classes. Per-library catalogs in `kinds/wireframe-classifiers/<library>.ts` (TS const declarations rather than JSON for type-safety; data still lives separately from the walker). On-demand pipeline: `findEntitiesByName` -> read `entity.body` -> tree-sitter parse -> walk JSX -> WireframeSpec. Recursive descent into in-tree imports (depth 3 default, max 6). Conditional / ternary / `.map()` rendering, Tailwind-className layout promotion, fragment flattening. 500-node walk cap. **No indexer extension.** Companion design doc `design/artifacts/react-introspection.html` still pending (prose; library catalog + rationale). |
