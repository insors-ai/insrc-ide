...
| 2     | Iteration: regenerate tool, Artifacts Pane, template-override commands       | todo |
...
| Item                                      | Status | Notes |
|-------------------------------------------|--------|-------|
| `artifact:regenerate` tool                | todo   | LLM calls it with `{ artifactId, edits }` -- no slash wrapper, same NL-only pattern as phase 1. |
| Revision history (last 5)                 | todo   |       |
| Artifacts Pane (Input + Pane + widget)    | todo   |       |
| Template-override commands                | todo   |       |
| `artifact:list_templates` tool            | todo   |       |
