...
| 2     | Iteration: regenerate tool, Artifacts Pane, template-override commands       | partial -- `artifact:regenerate` + `artifact:list_templates` tools, `TodosApi.updateItemMeta` extension, `persistence.appendRevision` with last-5 eviction, Artifacts editor pane (durable) + `insrc.artifacts.open` command all landed. Template-override commands (§2.3) deferred to a phase-2 follow-up -- needs a daemon RPC to seed user-overrides from bundled templates, which is net-new surface. Tests: +6 regenerate unit cases (67 pass total), smoke 11/11 green, IDE compile clean. |
...
| Template-override commands                | deferred | Descoped in this pass. Needs a daemon RPC to seed user-override files from bundled templates (workbench can't read daemon install paths). Users can still edit `~/.insrc/artifacts/templates/<kind>.html` manually -- the loader's mtime-keyed cache picks up edits live (phase 1 lint + three-layer resolution still in place). Follow-up ticket. |
