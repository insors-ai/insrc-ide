...
| CFG-based flow                            | partial | TS / TSX / JS + Python done via on-demand tree-sitter walks on the entity's `body`. Recognised nodes per language: branches (TS `if/switch`, Python `if/elif/else/match`), loops (TS `for/for-in/for-of/while/do-while`, Python `for/while`), exception flow (TS `try/catch/finally`, Python `try/except/finally` with multi-except collapse), terminators (TS `return/break/continue/throw`, Python `return/break/continue/raise`). Python `with` statements inline their body (no dedicated step). Mermaid `flowchart TD` with branch true/false labels + loop back-edges + dotted throw arrows. 200-node cap per function; overflow falls through to the phase-1 Kuzu CALLS approximation. **Go walker pending follow-up PR**. **No indexer changes.** |
...
| Cross-service callflow kind (`callflow`)  | partial | New 6th artifact kind for distributed call traces. **OTLP-JSON parser + `sequence` layout shipped**: closed `ArtifactKind` extended, `artifact:callflow` tool registered, bundled `callflow.html` template, canonical-span normaliser, parent-span traversal for participant resolution, `--x` arrows + Note-over for ERROR spans, dotted `-->>` + producer note for PRODUCER spans, caps at 20 services / 50 spans / 5s file-read timeout, free-text fallback. **Pending follow-up**: Jaeger JSON + Zipkin v2 JSON parsers, `layout: 'flowchart'` topology rendering, companion design doc `design/artifacts/callflow.html`. **No indexer dependency.** |
