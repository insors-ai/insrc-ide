...
| 55| Live-step stream needs a dedicated "activity console" bubble (not regular chat bubble)   | P1 | UI | **open** (Item 32b landed but uses the existing `.insrc-chat-message` structure with only dimmed styling. User observed the bubbles read as "just another chat message" rather than a live activity indicator. Also missing inner auto-scroll -- tokens append below the max-height cap and the user has to manually scroll the bubble body to see the latest output) |
| 56| Code-snippet formatting in theme-spec markdown still wrong                              | P1 | UI | **open** (Item 52's MarkdownRenderer migration renders the theme-spec content via VS Code's editor-backed renderer, but code fences inside the generated markdown don't format properly. Regression of the same symptom Item 52 partially fixed -- needs another pass to figure out whether the daemon's polished-section markdown is malformed or the CSS host class is overriding the renderer's own pre/code styling) |
