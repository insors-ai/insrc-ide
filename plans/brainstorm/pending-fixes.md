...
| 11| Post-ideation flow (converge / themes / spec / presentation)   | P1       | daemon+UI| **DONE** (2026-04-23 live test: clean end-to-end -- ideation -> convergence -> 4 theme specs (now with local-refine pass per Item 59) -> assemble -> presentation -> save -> handoff-proposal -> close. All gates rendered, no silent failures) |
...
| 32| Long agent steps feel disconnected -- user waits minutes on a single progress line with no token-level presence | P1 | daemon+UI | **DONE** (32a: task descriptions enriched with counts + N/M positions -- e.g. "Reviewing 9 ideas with Claude...", "Generating spec (3/7): Foo..."; 32b: `liveStep` IPC stream type wired through daemon->browser->chat panel, Anthropic `complete()` switches to streaming when `onToken` provided, all other providers already honored it. Visible-bubble polish tracked separately under Item 55) |
...
| 52| Markdown panes don't properly render code snippets (no syntax highlighting / styling)    | P1 | UI      | **DONE for theme-details** (migrated to VS Code's `MarkdownRenderer` via `IInstantiationService`; CSS selectors corrected to target `div[data-code]` per Item 56 since DOMPurify strips `class` from non-SPAN elements). Presentation-pane regression tracked separately under Item 57. |
...
| 55| Live-step stream needs a dedicated "activity console" bubble (not regular chat bubble)   | P1 | UI | **DONE** (new `.insrc-chat-live-console` class with boxed container, animated spinner header, uppercase `agent / step` label, monospace body with `max-height + overflow-y: auto`, and inner auto-scroll on each token append. Verified live on 2026-04-23 -- bubble appears on first token, auto-scrolls as tokens stream, removed cleanly on `done: true`) |
| 56| Code-snippet formatting in theme-spec markdown still wrong                              | P1 | UI | **DONE** (DOMPurify hook strips `class` from non-SPAN elements so the `class="code"` wrapper MarkdownRenderer emits never survived. Switched selectors to `.rendered-markdown div[data-code]` which the sanitizer passes through; tokenized spans get proper `<pre>`-like box + monospace + syntax highlighting. Verified live on 2026-04-23 -- code fences in the theme-spec review pane render with proper box + syntax coloring) |
| 57| Presentation pane's final-assembled spec doesn't format code blocks properly            | P1 | UI | **DONE** (chose Option 2: added `<pre>` / `<code>` rules to `BRAINSTORM_HTML_TEMPLATE`'s `<style>` block, scoped under `.container` so they don't leak into the IDE shell. Also added default `ul` / `ol` / `li` / `p` / `hr` / `blockquote` styling since the template previously only covered `.req-card` / `.theme-section` / etc. classes and left generic markdown elements to browser defaults) |
| 58| Assemble prompt renumbered theme sections (C-001/T-001/G-001), discarding stable theme IDs | P1 | daemon | **DONE** (changed design / implementation / testing assemble prompts to **preserve the existing theme IDs** -- `DES-TH-<hex>` / `IMP-TH-<hex>` / `TST-TH-<hex>` -- instead of renumbering. Theme IDs are assigned at convergence and have a 1:1 mapping to themes; renumbering broke traceability between the brainstorm themes and the final-doc sections. Requirements prompt (R-NNN per requirement, not per theme) left untouched) |
| 59| Theme-spec pipeline diverged from ideation (Claude polishedSection replaced local output; Claude `issues[]` + `suggestions[]` dropped) | P1 | daemon | **DONE** (added a `refine-theme-spec` step mirroring the ideation `refine`: after Claude reviews, a local LLM produces the final section incorporating Claude's polishedSection + issues + suggestions. Local is the final author; Claude is the reviewer. New generic `REFINE_THEME_SPEC_SYSTEM` prompt (category-agnostic), new `state.lastThemeReview` field persisted across resume, `refine-theme-spec` added to dispatch + `rebuildInFlightTask`. Fallbacks: parse-failure skips refine, empty/crashed refine falls back to Claude's polishedSection) |
| 60| `afterReviewThemeSpec` JSON-parse failed when Claude wrapped output in prose commentary | P0 | daemon | **DONE** (added a balanced-brace extraction strategy between the strict-JSON-parse and regex fallbacks: scans for the first `{`, walks matched braces respecting quoted strings + escapes, slices out the JSON substring and tries `JSON.parse` on just that slice. Handles Claude's common "Here is the reviewed section: { ... }. Hope this helps!" wrapper. Also added diagnostic logs -- `Claude flagged issues`, `Claude suggested improvements`, head/tail of raw output on extraction failure) |
