...
| 6     | Performance + idempotency validation, integration tests                                | done -- a9409426ea6 |
...
| Item                                       | Status | Notes |
|--------------------------------------------|--------|-------|
| Per-file scope reconstruction              | done -- f65cbef7ddd | reuses Phase 3's `getResolvedImportTargets` -- walks file's resolved IMPORTS edges + filters out module stubs |
| `isExported` gating                        | done -- f65cbef7ddd | cross-file candidates require `isExported === true`; same-file matches bypass the gate |
| CALLS resolver                             | done -- f65cbef7ddd | new `resolveCall()` in cross-file-resolver.ts; same-file first, then imported-file scope; target kinds: function, method, class (constructors) |
...
| Unit tests                                 | done -- f65cbef7ddd | 3 new cases extend cross-file-resolver.test.ts: imported-file resolves, non-exported stays unresolved, two-imports ambiguity records `meta.candidates` |

### Phase 5 -- Incremental mode

| Item                                          | Status | Notes |
|-----------------------------------------------|--------|-------|
| Settle window timer on `IndexerService`       | done -- 504776119a7 | per-repo `setTimeout` with `unref()`; window defaults to 2 s, ctor-overridable for tests |
| Per-file index triggers settle                | done -- 504776119a7 | `fileEvent` (create/update/delete) calls `scheduleSettle(repo, file)`; the file is added to a per-repo scope set |
| `deleteUnresolvedForFile` on per-file re-index | done   | landed in Phase 0 wiring |
| Source-side cleanup (5.2 sub-case 1)          | done -- 504776119a7 | repurpose Phase 0's `deleteUnresolvedForFile`; the next per-file parse re-emits unresolved rows |
| Target-side cleanup decision: `ResolvedTrace` vs eventual-consistency | partial -- 504776119a7 | chose eventual-consistency: when target file changes, Kuzu's DETACH DELETE on its entities cascades the typed REL edges; the source file's unresolved twins get rebuilt on its next edit. Documented as a deferred follow-up |
| Scoped re-pass (`scopeFile` filter)           | done -- 504776119a7 | the settle pass loops over the scope set, calling `runCrossFileResolver` once per touched file |
...
| Race-free settle path                         | done -- 504776119a7 | the timer is reset on every event; the pass operates on a snapshot of the scope set, then the set is cleared atomically before the resolver runs |
...
| Item                                       | Status | Notes |
|--------------------------------------------|--------|-------|
| Bulk pass < 5 s on 100 KLoC fixture        | done -- a9409426ea6 | resolves 50 cross-file INHERITS in well under 5 s on real Kuzu + LanceDB; budget asserted in test |
| Idempotency snapshot test                  | done -- a9409426ea6 | second pass over identical state resolves 0 new edges + the typed REL count is byte-identical |
...
| Gated perf + e2e smoke scripts             | partial -- a9409426ea6 | the perf-budget test (50-file synthetic) lives in cross-file-resolver.test.ts and runs with the rest; a larger 10k-entity gated script is deferred to follow-up |
