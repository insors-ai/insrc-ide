...
| F6 | Validation environment: analyzer + indexer compete for single Kuzu connection | F2 root cause; surfaces during any concurrent indexer-running `/code-analyze` test. Not a bug per se, but inflates apparent latency for both. | Either run analyzer tests after `full index complete`, or fix the connection-pool architecture (F2(c)) -- which is exactly what F7 below proposes. | **Partially addressed by F7's dual Connection** (`6512d43f106` adds `db.graphReader`); the analyzer runner still uses `db.graph` writer for tool calls -- per-caller cutover is follow-up work. |
...
These are tracked here rather than as separate plan-doc commits so the validation context stays grouped with Phase 1's acceptance section. Convert to commits / per-issue work as bandwidth permits; F1 + F3 remain the two highest-leverage low-risk fixes still pending. F4 should self-resolve once F1 is fixed (most retry-with-hint pressure comes from F1 failures).
