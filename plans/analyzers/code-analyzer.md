...
- [plans/analyzers/code-analyzer-skills.md](./code-analyzer-skills.md) — **the analyzer-logic layer** (drafted 2026-05-09). Decomposes the per-task analyzer runner described in this plan into a graph of fine-grained skills. This plan stays as the substrate / product surface (orchestrator state machine, slash command, Report Pane, citations invariant, gate widget); the skills plan owns analyzer logic. Critical-path note: that plan's Phase 3 is the prerequisite for [data-analyzer-skills §3 + §4](./data-analyzer-skills.md) -- ship Phase 3 first if the goal is unblocking the data-analyzer cutover.
