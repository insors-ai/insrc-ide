...
| 5c.1 | dependency: correlation.numeric-pairwise | partial | Both `.rdbms` and `.file` shipped. Math in `data.correlation.numeric-pairwise.algo.ts` (Pearson + Spearman + classification). **Still partial** because the math is sample-based; precise full-table corr() needs Phase 0.4 `db_correlation_matrix` (Track C). Transport ports done. |
...
| 5c.4 | dependency: dependency.co-null-pattern | partial | Both `.rdbms` and `.file` shipped. Math in `data.dependency.co-null-pattern.algo.ts`. **Still partial** because precise full-table counts need a count_where aggregate (Track C); transport ports done. |
...
| 5d.1 | quality scorecard: quality.completeness | partial | Both `.rdbms` and `.file` shipped. Math in `data.quality.completeness.algo.ts` (auto-discovers columns; packs `count(*)` + `count_non_null` per column into one aggregate round-trip; up to 31 columns per call). |
| 5d.2 | quality scorecard: quality.uniqueness | partial | Both `.rdbms` and `.file` shipped. Math in `data.quality.uniqueness.algo.ts` (per-column `distinctCount/nonNullCount` + single-column PK-candidate detector; cap at 15 columns per call due to `1 + 2N <= 32` aggregate-spec budget). Multi-column PK candidates still pending |
| 5d.3 | quality scorecard: quality.validity | partial | Both `.rdbms` and `.file` shipped. Math in `data.quality.validity.algo.ts` (caller-supplied JS regex pattern, samples up to 50 values, returns match / mismatch counts + match rate + up to 3 examples each). Pattern compiled once with `new RegExp(pattern)` so a malformed pattern fails fast. **Type / domain / CHECK-constraint** validity (the rest of the plan's row) deferred -- needs per-dialect catalog introspection. **Integrated into 5d.6 scorecard** via the optional `validityPatterns` map -- columns the caller patterns get folded into the composite, others stay null and are excluded from the per-column score |
...
| 5e.1 | sensitivity: pii.detect-patterns | partial | Both `.rdbms` and `.file` shipped. Math + catalog in `data.pii.detect-patterns.algo.ts` (samples up to 50 values, applies anchored regex set: email / ssn-us / phone-us / credit-card / jwt / ipv4 / iban / aws-access-key / github-token / uuid; returns per-pattern hit count + rate + up to 3 examples). Provider affinity `local`. KV variant pending; address detection skipped (no clean regex) |
