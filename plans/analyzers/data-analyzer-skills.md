...
| 5a.3 | quality-profile: profile.temporal | partial | Both `.rdbms` and `.file` shipped (count + non-null + null + distinct cardinality). Math in `data.profile.temporal.algo.ts`. **Still partial** because min / max range + gap detection + period inference remain deferred -- gated on a type-aware aggregation surface (per Track C). The transport ports are done; the math gap is what keeps "partial" |
| 5a.4 | quality-profile: profile.text | partial | Both `.rdbms` and `.file` shipped (server-side cardinality + null rate; sample-based length stats min / max / avg / median + empty-string count over up to 50 sampled values). Math in `data.profile.text.algo.ts`. **Still partial** because encoding detection (deferred -- driver-level byte access not exposed) and generalized regex pattern inference (covered separately by `pii.detect-patterns.rdbms` 5e.1 + a future general-pattern skill) aren't part of this skill |
...
| 5b.5 | distribution: distribution.normality-test | partial | `data.distribution.normality-test.rdbms` shipped (Jarque-Bera test). Mean / stddev pulled server-side; skewness + kurtosis computed over the 50-row sample (`db_sql_aggregate` doesn't expose `skewness` / `kurtosis` aggregates yet -- small future extension). JB statistic + chi-squared(2) closed-form p-value (`exp(-JB/2)`) + verdict (`normal` / `non-normal` / `inconclusive`) + per-column interpretation (skewness direction, tail weight). `min-sample-size: 50` precondition gates verdict. Picked JB over Shapiro-Wilk because Royston's SW approximation is ~60 lines of careful coefficient math; JB is the same n>=50 sweet spot |
| 5b.6 | distribution: distribution.heavy-tail-check | partial | `data.distribution.heavy-tail-check.rdbms` shipped (atomic; sample-based kurtosis verdict). Returns `heavy-tailed / light-tailed / mesokurtic / inconclusive` plus excess-kurtosis value (kurtosis - 3). Default threshold ±1.0. Interpretation line steers callers toward MAD over IQR / Z-score on heavy-tailed columns. Sample n=50 (precision-limited vs the plan's n>=200 ideal until a `kurtosis` aggregate function lands). Pairs naturally with normality-test (5b.5) -- both compute moments from the same sample shape |
| 5b.7 | distribution: distribution.modes | partial | `data.distribution.modes.rdbms` shipped (atomic; sample-based histogram + local-maxima detection). Pulls 50 values, bins into a fixed-width histogram (default 10 bins), smooths with 3-point moving average, finds local maxima with prominence > 50% of global peak, collapses adjacent-peak plateaus. Modality verdict: `unimodal / bimodal / multimodal / inconclusive`. Sample-based; full-table histogram via `db_sql_histogram` (Phase 0.2) is the natural upgrade -- the mode-finding logic doesn't change |
| 5c.1 | dependency: correlation.numeric-pairwise | partial | `data.correlation.numeric-pairwise.rdbms` shipped (atomic; pairwise Pearson + Spearman over a 50-row sample). Auto-discovers numeric columns via `db_sql_describe` (lowercase substring rules: int / decimal / numeric / number / real / float / double / money); caller-supplied `columns` skip the filter. Cap: 15 columns / 105 unordered pairs. For each pair: skip rows where either column is null; require >=5 paired non-null rows; compute Pearson over centred values, Spearman as Pearson on average-rank-transformed values (Brown 1988 tie handling). Each pair gets a classification ladder: strong/moderate/weak (positive\|negative) at \|r\| thresholds 0.7 / 0.4 / 0.2. Returns full pair table sorted by \|Pearson\| desc + top-3 positive / top-3 negative shortlists. Pearson + Spearman together let callers distinguish linear (Pearson ≈ Spearman) from monotonic-curved (Spearman much larger). End-to-end verified: synthetic 30-row sample with `b=2a+noise` -> Pearson +0.999; `c=100-a+noise` -> Pearson -0.998; independent `e` -> all \|Pearson\| < 0.1. Sample-based; precise full-table corr() needs Phase 0.4 `db_correlation_matrix`. Pairs with `correlation.categorical-pairwise.rdbms` (5c.2 -- pending) for the categorical analogue |
| 5c.2 | dependency: correlation.categorical-pairwise | partial | `data.correlation.categorical-pairwise.rdbms` shipped (atomic; pairwise Cramér's V over a 50-row sample). Auto-discovers categorical columns by declared SQL type (text / varchar / char / nvarchar / character / enum / bool) AND drops high-cardinality columns (>25 distinct values in sample, configurable via `maxDistinctPerColumn`) so ID-like columns (user_id, uuid) don't trivially saturate V. Cap: 15 columns / 105 unordered pairs. For each pair: build contingency table, compute χ² = Σ(O-E)²/E, V = sqrt(χ²/(n×min(rows-1,cols-1))). Classification ladder by \|V\|: strong (>=0.7) / moderate (>=0.4) / weak (>=0.2) / none / inconclusive (n<5 OR fewer than 2 distinct on either side). Returns full pair table + top-5 associated shortlist + a list of dropped columns (transparency). End-to-end verified on a 40-row fixture with deterministic `region -> product_class` mapping, independent `channel`, and a 40-distinct-value `user_id`: V(region, product_class)=0.833 (strong), V(region, channel)=0.295 (weak), V(product_class, channel)=0.286 (weak), `user_id` dropped. v1: sample-based; precise full-table contingency needs a future `db_sql_groupby_two` tool. No bias correction (Bergsma 2013) -- accuracy fine for n=50. Pairs with `correlation.numeric-pairwise.rdbms` (5c.1) for the numeric flow |
| 5c.3 | dependency: dependency.functional | partial | `data.dependency.functional.rdbms` shipped (atomic; pairwise FD check over 50-row sample). For every ordered pair (A, B), groups rows by A, counts distinct B per group, returns `determinationScore` + a `determines` flag (score >= 0.95 AND informativeGroups >= 3, where informativeGroups = groups with >= 2 rows -- trivially-consistent singletons don't qualify). Top-3 violation examples per pair. Sample-based; precise full-table FD checks need a count-distinct-grouped aggregate not yet shipped. Cap: 10 columns / 90 ordered pairs per call |
...
| 5c.5 | dependency: cardinality.join-key | partial | `data.cardinality.join-key.rdbms` shipped (atomic; cross-table cardinality classification + orphan estimate). Four parallel tool calls (aggregate + distinct on each side) derive `1:1 / 1:N / N:1 / N:M / unknown` from per-side uniqueness ratios; orphan detection compares up to 1000 distinct values per side. Limitations: same-connection only (cross-connection joins out of scope); columns with > 1000 distinct values produce lower-bound orphan counts (`valueSetTruncated` flag) |
