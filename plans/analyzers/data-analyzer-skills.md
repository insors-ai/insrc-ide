...
| 5b.4 | distribution: distribution.outliers-mad | partial | Both `.rdbms` and `.file` shipped. Math in `data.distribution.outliers-mad.algo.ts` (modified Z-score via MAD; default threshold 3.5). Sample-based MAD (server-side MAD needs a two-pass aggregate not yet shipped) -- transport ports done; this math gap is what keeps "partial". |
| 5b.5 | distribution: distribution.normality-test | partial | Both `.rdbms` and `.file` shipped. Math in `data.distribution.normality-test.algo.ts` (Jarque-Bera; chi-squared(2) closed-form p-value). Skewness/kurtosis still sample-based; gap is the aggregate tool surface (Track C). |
| 5b.6 | distribution: distribution.heavy-tail-check | partial | Both `.rdbms` and `.file` shipped. Math in `data.distribution.heavy-tail-check.algo.ts` (sample kurtosis verdict, default threshold 1.0). Sample-based at n=50 vs the plan ideal n>=200; transport ports done, gap is on the aggregate-tool side. |
| 5b.7 | distribution: distribution.modes | partial | Both `.rdbms` and `.file` shipped. Math in `data.distribution.modes.algo.ts` (sample-based histogram + smoothing + local-maxima detection + plateau-collapse). Full-table histogram (5b.1 + 0.2 db_sql_histogram) remains the natural upgrade -- transport ports done, math gap is on the tool side. |
