...
| 5d.3 | quality scorecard: quality.validity | done (Gap 1) -- Gap 2 explicitly deferred | Both `.rdbms` and `.file` shipped. Default `mode: 'sample'`: math in `data.quality.validity.algo.ts` (caller-supplied JS regex pattern, samples up to 50 values, returns match / mismatch counts + match rate + up to 3 examples each). **Phase 5d.3 Gap 1 (2026-05-07): Option 1 chosen + shipped** -- new opt-in `mode: 'full-table'` issues count + count_non_null + count_where(regex) aggregates for an exact match-rate over the full non-null population. Required substrate landed alongside: WhereClause op enum extended with `'regex' / 'not regex'`; `Dialect.regexPredicate` callable wired per-dialect (Postgres `~` / `!~`; MySQL + SQLite `REGEXP` / `NOT REGEXP`; Oracle `REGEXP_LIKE`; ClickHouse `match`); MSSQL leaves `regexPredicate` undefined and `compileWhere` raises a clear "regex not supported on this dialect" error rather than emit invalid SQL. SQLite driver registers a deterministic JS REGEXP user function on connect (better-sqlite3's `db.function`) so `WHERE col REGEXP pattern` works; otherwise the engine errors with "no such function: REGEXP". Output gained `source: 'sample' \| 'full-table'`, `totalRows: number \| null`, `nonNullCount: number \| null`. In full-table mode the `examples` arrays are empty (callers wanting examples either follow up with a sample call or stay in `mode: 'sample'`). **Integrated into 5d.6 scorecard** via the optional `validityPatterns` map (back-compat preserved). Coverage: 7 new compileWhere regex test cases in `rdbms-common.test.ts`, 1 SQLite-end-to-end test confirming the REGEXP user-function plumbing, 5 dedicated full-table-mode skill tests in `daemon/skills/__tests__/quality-validity-full-table.test.ts` (high / medium / low confidence paths + sample-mode back-compat + xlsx path forwarding). **Gap 2 (CHECK-constraint introspection) explicitly deferred** -- the chosen Option 1 trade-off. Format-shape validation already covered by 5d.4 conformity's built-in catalog (13 canonical formats); CHECK-constraint auto-discovery is left for a future plan that ships a structured CHECK-clause parser (range / IN-list / nullness lifted to WhereClause; exotic predicates fall through with a "could not parse" note). The Option 2 raw-SQL escape hatch (`count_where_raw`) was rejected to preserve the structured-WhereClause boundary. |
...
| 5f.1 | drift over windows: drift.distribution | done | Both `.rdbms` and `.file` shipped, with sample + full-table modes (Track-C swap landed 2026-05-07). Default `mode: 'sample'`: 50 rows per window, JS divergence in JS over the samples. Opt-in `mode: 'full-table'`: 4-call protocol -- (1) parallel min/max/count_non_null per window, (2) parallel N count_where bucket counts per window over shared edges derived from joint min/max. Bucket disambiguation uses synthetic `bucket_<i>` column tags on each count_where spec (count_where skips column-validation per `rdbms-common.ts`, so synthetic tags are accepted by every dialect; needed because `countWhereSignature` strips literal values, leaving 10 same-shape predicates with identical signatures otherwise). DriftWhereClauseIn op enum widened to include `<` / `<=` / `>` / `>=` / `between` / `is not null` so callers can express time-bounded windows in either mode. JS divergence math reused via shared `assembleDriftFromBins` helper; verdict interpretation tagged `(full-table)` when from server-side counts. Empty-window case detected at the bounds phase (medium confidence + clear note, no pointless bucket queries). Output schema gained `source: 'sample' \| 'full-table'`. Coverage: 5 dedicated full-table tests in `daemon/skills/__tests__/drift-distribution-full-table.test.ts` (identical / divergent / empty-window / tool-error / file xlsx path forwarding) |
| 5f.2 | drift over windows: drift.volume | done | Both `.rdbms` and `.file` shipped. Math in `data.drift.volume.algo.ts` (row-volume comparison between two windows; caller supplies two `WhereClause[]` filters; two parallel `count_non_null` aggregates with the where filters return absolute / relative / percent change + ratio + verdict ladder: stable <10% / minor-change 10-25% / significant-drop <=-25% / significant-spike >=+25% / inconclusive when countA=0). `countColumn` auto-detected from describe -- prefers PK, falls back to first non-nullable column. **Earlier bug fixed:** the original ship passed `where` to `db_sql_aggregate` whose input schema rejected it. Resolution: extended `db_sql_aggregate` (and `db_file_aggregate`) to accept `where: WHERE_SCHEMA`, threaded through `compileAggregate` -- benefits any future windowed skill. The smoke harness was also hardened to validate skill tool-call inputs against the registered tool's schema (would have caught this regression class; verified with a dedicated harness test that intentionally passes a phantom field). Already full-table by construction (server-side count_non_null with WHERE per window); the `partial` label was stale and got flipped to `done` 2026-05-07. Pairs with `data.drift.distribution` (5f.1) -- volume drift = how much got produced; distribution drift = whether the shape of values changed. |
| 5f.3 | drift over windows: anomaly.change-point | shipped (sample-only by design) -- full-table needs new substrate | `data.anomaly.change-point.rdbms` shipped (atomic; single-change-point detection on sorted (timestamp, value) sample). For every interior split index k computes the standardised mean shift `|leftMean - rightMean| / pooledStddev`; returns the k that maximises it. Verdict: clear-change (>=2σ) / subtle-change (>=1σ) / no-change / inconclusive. Sample-based at n=50 (min n=10). End-to-end verified: synthetic step at index 20 (50 -> 100 with σ=3 noise) -> changePointIndex=20, leftMean=49.5, rightMean=99.2, standardisedShift=2.07σ, verdict 'clear-change'. **Full-table port is NOT mechanical** -- the algorithm scans every interior split index over a sorted timestamp axis, which is fundamentally a multi-pass operation that doesn't decompose into the existing flat aggregate primitives. Server-side change-point detection would need either (a) a new `cumulative_sum` / `running_mean` window-function aggregate so the per-split-mean delta can be computed in SQL, or (b) the skill issues O(n) `count_where` aggregates over different cut points (prohibitive at scale). Both options are substrate work. Pairs with `drift.distribution.rdbms` (which compares two pre-chosen windows; this skill *finds* the best split) |
| 5g.1 | timeseries: timeseries.trend | done | `data.timeseries.trend.rdbms` shipped with sample + full-table modes. Default `mode: 'sample'`: pure-JS OLS over a 50-row (timestamp, value) sample. Opt-in `mode: 'full-table'` (Phase 5g.1 substrate landed 2026-05-07) delegates to the new `db_sql_temporal_trend` tool. Substrate: `Dialect.epochExpr` (per-dialect timestamp→epoch-seconds: PG/DuckDB `EXTRACT(EPOCH FROM)`, MySQL `UNIX_TIMESTAMP`, SQLite `unixepoch()`, MSSQL `DATEDIFF_BIG`, Oracle date-arithmetic, ClickHouse `toUnixTimestamp`); native REGR_SLOPE / REGR_INTERCEPT / REGR_R2 path on PG/DuckDB/Oracle; expression-based path (single-row SUM moments → centred-form Sxx/Sxy/Syy → JS slope/intercept/R²) on MySQL/SQLite/MSSQL/ClickHouse. Output gained `source: 'sample' \| 'full-table'`. Coverage: 12 substrate tests in `rdbms-common.test.ts`, 1 SQLite e2e, 4 dedicated full-table-mode skill tests. Pairs with `drift.distribution.rdbms` for a complete temporal-analysis story (trend + drift) |
| 5g.2 | timeseries: timeseries.seasonality | shipped (sample-only by design) -- full-table genuinely not tractable | `data.timeseries.seasonality.rdbms` shipped (atomic; autocorrelation-peak detection on a detrended sample). Pulls a 50-row (timestamp, value) sample, sorts by timestamp, removes the OLS trend, computes `r(k)` for k=2..n/2, finds local maxima above the 95% Bartlett band (1.96/sqrt(n)). **Lag 1 is excluded from peak detection** -- its implicit left-neighbor is `r(0)=1`, so for any smooth signal r(1) appears as a "peak" of smoothness, not periodicity. Returns top-5 peaks + best peak's lag + estimated time-span + human-readable form. Verdict ladder: seasonal (\|r\|>=0.5) / weakly-seasonal (\|r\|>=0.3) / aperiodic / inconclusive. End-to-end verified on a synthetic sin-series: bestPeriodLag=10 (5 cycles in 50 hrs), r=0.79, verdict 'seasonal'. **Full-table port stays explicitly out-of-scope.** Autocorrelation at lag k requires correlation between the column and its k-shifted self -- doing this for every k from 2..n/2 over millions of rows would mean (n/2 - 1) full table scans, each computing a correlation between paired (y[t], y[t+k]) values. That doesn't decompose into the existing flat aggregate primitives, and even with LAG it would need O(n) parameterised window functions. The right substrate would be a dedicated FFT / periodogram primitive (engine-specific; PG has `tablefunc.crosstab` workarounds, DuckDB has none, MSSQL/SQLite none), which is far beyond Phase 0's scope. **Decision: stay sample-only.** A 50-row autocorrelation captures the dominant period for any sufficiently regular series; for finer-resolution full-table seasonality detection, callers should pre-aggregate (e.g. hourly rollups) and re-sample. Pairs with `data.timeseries.trend.rdbms` (5g.1) |
| 5g.3 | timeseries: timeseries.stationarity | done | `data.timeseries.stationarity.rdbms` shipped with sample + full-table modes. Default `mode: 'sample'`: pure-JS Dickey-Fuller on a 50-row sample with MacKinnon (1996) n=50 critical values (-3.58 / -2.93 / -2.60). Opt-in `mode: 'full-table'` (Phase 5g.3 substrate landed 2026-05-07) delegates to the new `db_sql_dickey_fuller` tool, which uses a CTE + LAG window function to materialise (y[t], y[t-1]) pairs server-side, then aggregates the SUM moments needed to derive β + SE(β) + t-statistic. JS-side compares the t-stat to MacKinnon **asymptotic** critical values (-3.43 / -2.86 / -2.57) since for full-table runs n is the entire paired population. Universally supported on dialects with CTE + LAG (PG / DuckDB / MySQL 8+ / SQLite >=3.25 / MSSQL / Oracle); ClickHouse stub throws. Output gained `source: 'sample' \| 'full-table'`. Coverage: 3 dedicated full-table tests + sample-mode smoke fixture. Pairs with `data.timeseries.trend.rdbms` (5g.1) -- trend captures slope, stationarity captures whether residuals are mean-reverting vs random walk |
| 5g.4 | timeseries: timeseries.gap-analysis | done | `data.timeseries.gap-analysis.rdbms` shipped with sample + full-table modes. Default `mode: 'sample'`: 50-row median + top-10 gaps + regularity score. Opt-in `mode: 'full-table'` (Phase 5g.4 substrate landed 2026-05-07) delegates to the new `db_sql_temporal_gap_stats` tool (two-phase protocol). Phase 1: CTE + LAG over the sorted timestamp column, then PERCENTILE_CONT(0.5) for the median delta + min/max epoch + n_total / n_deltas. Phase 2: with the median known, count regular deltas (within ±50% of median) and gap deltas (> gapRatio × median) via SUM-of-CASE, plus a separate query pulling the top-N gap deltas by ratio descending. Universally supported on dialects with CTE + LAG + PERCENTILE_CONT; ClickHouse stub throws. Skill converts seconds→milliseconds at the boundary so the public output schema stays consistent with sample mode. Output gained `source: 'sample' \| 'full-table'`. Coverage: 3 dedicated full-table tests + sample-mode smoke fixture. Pairs with the rest of 5g -- regression / autocorrelation / DF tests all assume regular sampling, so this skill is the canonical precondition |
| 6.1 | synthesis: synth.target-description | pending -- **open question** | Plan body says "needs an LLM call (narrative summary)" but the LLM-routing table at the bottom lists synthesis family as `n/a (no LLM call)`. Resolve before implementing: (a) is 6.1 a true outlier in the synthesis family with cloud affinity, or (b) is it a misclassification and should ship as a deterministic template like 6.2 / 6.5-6.8? Also unclear what input shape it consumes (SchemaDescription? composite over describe + profile?). Skipping until decided |
...
| 6.3 | synthesis: synth.drift-report | **deferred** -- transitive on Phase 4 | Renderer for the schema-drift outputs in 4.1-4.3; deferred until those land. (5f drift skills emit a different, narrower DriftResult; if a 5f-flavoured drift renderer is wanted standalone, ship as a separate slice rather than coupling to 6.3) |
| 6.4 | synthesis: synth.er-diagram | **deferred** -- transitive on Phase 4 | Mermaid ER diagram over multi-table SchemaDescription[]; needs Phase 4 aggregation point or a new cross-table introspection skill. Defer with the rest of Phase 4 |
...
+ type-aware values for temporal min/max), plus the **5g substrate
landed 2026-05-07**: `Dialect.epochExpr` (per-dialect timestamp →
epoch-seconds), native + expression regression paths, and the new
dedicated tools `db_sql_temporal_trend` (5g.1), `db_sql_dickey_fuller`
(5g.3), `db_sql_temporal_gap_stats` (5g.4). The latter two use a
CTE + LAG window function pattern that's universally supported on
modern dialects (PG / DuckDB / MySQL 8+ / SQLite >=3.25 / MSSQL /
Oracle); ClickHouse stubs throw. All Track C swaps expressible via
the shipped substrate are landed. The remaining deferred rows below
are limited to skills whose math genuinely doesn't decompose into
flat or windowed aggregates (anomaly.change-point's multi-pass
scan; seasonality's per-lag autocorrelation).
...
| 5g.1 timeseries.trend -- full-table | new `db_sql_temporal_trend` tool + Dialect.epochExpr | **done (2026-05-07)**. Native REGR_SLOPE/REGR_INTERCEPT/REGR_R2 on PG/DuckDB/Oracle; expression-based SUM-moments path on MySQL/SQLite/MSSQL/ClickHouse. Skill gained opt-in `mode: 'full-table'`. |
| 5g.3 timeseries.stationarity -- full-table | new `db_sql_dickey_fuller` tool (CTE + LAG) | **done (2026-05-07)**. CTE + LAG materialises (y[t], y[t-1]) pairs server-side; orchestrator computes β + SE(β) + t-statistic in JS. Skill compares to MacKinnon asymptotic critical values. |
| 5g.4 timeseries.gap-analysis -- full-table | new `db_sql_temporal_gap_stats` tool (CTE + LAG + PERCENTILE_CONT) | **done (2026-05-07)**. Two-phase protocol: median + min/max via PERCENTILE_CONT, then bucket counts (regular vs gap) + top-N gap deltas. Skill converts seconds→milliseconds at the boundary. |
| 5g.2 timeseries.seasonality -- full-table | not naturally tractable without FFT/periodogram primitive | **out-of-scope by design**. Per-lag autocorrelation over a full population needs O(n/2) full-table scans; the right substrate is a dedicated FFT primitive that no shipped engine surfaces uniformly. Sample-only is the right design choice; recommend pre-aggregation for callers who need finer resolution. |
