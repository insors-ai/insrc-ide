...
| B.0.1 | Pool split: query engine ≠ storage engine | done | discovered Phase A had silently moved the graph onto the data-driver's `:memory:` DuckDB, dropping graph state on every restart. Fixed by splitting into two singletons: `daemon/db/duckdb-pool.ts` (in-memory query engine, future data-driver attaches) + `daemon/db/duckdb-storage-pool.ts` (file-backed `~/.insrc/duckdb.db`, graph + vector + user state). GraphClient routed onto the storage pool. Storage default memory_limit raised to 2 GB (DuckDB preallocates buffer-pool space proportional to memory_limit at the first FLOAT[N] CREATE TABLE -- 512 MB was too small to even hold the schema with N=2560). Tests use `setStorageDuckDBPath(':memory:')` for isolation; one test exercises file-backed persistence across close/reopen |
