...
| **P10** | Phase 6 cutover: `/code-analyze` routes through `code.answer-question` | Clean cutover (no feature flag). New thin adapter `agent/tasks/code-analyzer/answer-question-section.ts` calls `runL2Skill('code.answer-question', ...)` per `PlannedAction` and stitches the L2 skill's `sections[]` into one section markdown. Orchestrator's per-section `runDiscoveryFlow` call swapped to the adapter. Legacy modules deleted in the same commit: `discovery-flow`, `write-from-evidence`, `claim-grounding-reviewer`, `meta-narrative-detector`, `cycle-memory`, `execute-step`, `content-gen/discovery-plan-actions`, 9 flow prompts (`gather` / `write` / `patch` / `discovery-expand` / `discovery-review` / `execute-step` / `prose-review` / `claim-grounding` / `write-structured`), 18 orphan section MDs (coverage-angles, planner-context, role-gather/write/patch-*, skill-catalog-cloud / -usage / -glossary, etc.), 13 golden snapshots, and all associated tests. `PromptFlow` enum collapses to `'review'`; `normalizeTier` + `loadPatchPrompt` removed. Net: -39 prompt MDs, -3866 LOC. | done |
...
| --  | `code.class.locate-references` | (mechanical cache wiring -- no per-skill plan doc) | `code.class.locate-references.ts` | 24h TTL; key=`className::repoPath?::scope::language?`. |
| --  | `code.compare.entity-versions` | (mechanical) | `code.compare.entity-versions.ts` | 7d TTL (immutable git history); key=`entityId::baseRef::headRef`. |
| --  | `code.compare.impl-vs-doc` | (mechanical) | `code.compare.impl-vs-doc.ts` | 24h TTL; key=`className::repoPath::docPath`. |
| --  | `code.compare.signature` | (mechanical) | `code.compare.signature.ts` | 7d TTL; **symmetric key** sorted lexicographically so cmp(A,B)/cmp(B,A) share one entry. |
| --  | `code.entity.search-by-vector` | (mechanical) | `code.entity.search-by-vector.ts` | 1h TTL (vector index may rebuild); key=`query::filter::sortedClosure::limit`. |
| --  | `code.entity.callers` | (mechanical) | `code.entity.callers.ts` (shared file) | 24h TTL; key=`entityId`. |
| --  | `code.entity.callees` | (mechanical) | `code.entity.callers.ts` (shared file) | 24h TTL; key=`entityId`. CALLERS/CALLEES constants prefixed to disambiguate. |
| --  | `code.migration.extract-history` | (mechanical) | `code.migration.extract-history.ts` | 24h TTL; key=`repoPath::tool`. |
| --  | `code.orm.resolve-model` | (mechanical) | `code.orm.resolve-model.ts` | 24h TTL; key=`model::orm::repoPath?::scope?`. |
| --  | `code.source.grep` | (mechanical) | `code.source.grep.ts` | 1h TTL (file contents drift); key=`path::pattern::maxHits`. |
| --  | `code.source.repo.describe` | (mechanical) | `code.source.repo.describe.ts` | 24h TTL; key=`repoPath`. |
