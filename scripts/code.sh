...
function code() {
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		...
		# If the renamed binary doesn't exist, symlink from the default Electron binary
...
function code-wsl()
{
	...
	# in a wsl shell
	local WIN_NAME=`node -p "require('./product.json').nameLong"`
	ELECTRON="$ROOT/.build/electron/$WIN_NAME.exe"
	if [ ! -f "$ELECTRON" ]; then
