...
const skill: Skill<RdbmsSampleRowsInput, RdbmsSampleRowsOutput> = {
	...
	async execute(input, deps): Promise<SkillResult<RdbmsSampleRowsOutput>> {
		...
		const value: RdbmsSampleRowsOutput = {
			target:         data.target,
...
// ---------------------------------------------------------------------------
//
// Row samples reflect live data; 1h TTL keeps the cache fresh while
// still short-circuiting repeat sample-rows requests within an
// analyzer loop.

const OWNER_ID: OwnerId = 'skill:data.source.rdbms.sample-rows';
const NAMESPACE = 'rdbms-samples';
const TTL_MS = 60 * 60 * 1000;
const INTERESTED_TRIGGERS: readonly BootstrapTriggerKind[] = ['connection-add', 'refresh', 'manual'];

function cacheKey(input: RdbmsSampleRowsInput): string {
	const limit = clampLimit(input.limit);
...
const CONTEXT_SLOTS: readonly ContextSlotRequest[] = [
	{
		...
		namespace: NAMESPACE,
		query: (req) => {
			const task = (req.task ?? {}) as RdbmsSampleRowsInput;
...
const MEMORY_SCHEMA: readonly NamespaceSpec[] = [
	{
		namespace:   NAMESPACE,
		valueType:   'RdbmsSampleRowsOutput',
...
function readCachedSample(input: RdbmsSampleRowsInput, deps: SkillDeps): RdbmsSampleRowsOutput | undefined {
	const slot = deps.context?.slots.get('cached-sample');
	if (slot === undefined || slot.length === 0) { return undefined; }
	const hit = slot[0] as MemoryEntry<RdbmsSampleRowsOutput>;
	if (hit.value === undefined) { return undefined; }
	if (hit.key !== cacheKey(input)) { return undefined; }
	return hit.value;
}

function pinSample(input: RdbmsSampleRowsInput, value: RdbmsSampleRowsOutput, deps: SkillDeps): void {
	if (deps.workingState === undefined) { return; }
	const ref = deps.workingState.append({
		source:  { kind: 'tool', toolId: 'db_sql_sample' },
		payload: value,
		claims:  [`sample-rows:${cacheKey(input)}`],
		confidence: 0.95,
	});
