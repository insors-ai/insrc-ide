...
function buildSystemPrompt(catalog: readonly CatalogEntry[]): string {
	return [
		...
		'Your job is to call read-only skills to investigate the repository, NOT to write prose.',
		'',
		'Each turn:',
		'  - Decide which skill to invoke next OR signal that you have enough evidence.',
		'  - Use the skill_invoke / skill_describe / skill_load_page meta-tools.',
		'  - Describe a skill (`skill_describe({ id })`) before invoking it the first time.',
		'  - The orchestrator AUTOMATICALLY captures a structured summary of each invoke result',
		'    after the call -- you do not need to interpret results in your text.',
		'',
		'Stop condition:',
		`  - When you judge you have enough evidence to cover the section objective + review criteria,`,
		`    emit exactly the text \`${EVIDENCE_COMPLETE_SENTINEL}\` (with NO tool calls in that turn).`,
		'  - Otherwise keep invoking skills.',
		'',
		'Hard rules:',
		'  - Do NOT write prose for the report. That happens in a SEPARATE write phase after gathering.',
		'  - Keep your text turns SHORT -- one sentence stating what you intend to investigate next.',
		`  - When done, emit ONLY the sentinel: \`${EVIDENCE_COMPLETE_SENTINEL}\` (no extra commentary).`,
		'',
