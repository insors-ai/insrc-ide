...
export const serializeStep: AgentStep<PlannerState> = {
  ...
  async run(state, ctx): Promise<StepResult<PlannerState>> {
    ...
    // Mirror the finalised plan into a todos list owned by 'planner'
    // (plans/todo-framework.md Phase 7 -- planner consumer). Best-
    // effort: failure logs to ctx.progress and doesn't block the
    // serialize step. The list lives under owner = 'planner' so the
    // implementation family can pick it up via the standard transfer
    // primitive when an implementation run starts on the same
    // session. Each plan step becomes a TodoItem with the planner's
    // step id stashed in meta so a future consumer can correlate.
    if (ctx.todos !== undefined && ctx.sessionId !== undefined && state.plan !== null) {
      try {
        const list = await ctx.todos.createList({
          sessionId: ctx.sessionId,
          title: state.plan.title,
          description: state.plan.description,
          body: `Plan id: ${state.plan.id} (${state.inferredPlanType}). Generated by the planner family; ready for transfer to the implementation family.`,
        });
        for (const step of state.plan.steps) {
          await ctx.todos.addItem(list.id, {
            ...
            meta: {
              plannerStepId: step.id,
              plannerPlanId: state.plan.id,
              dependencies: step.dependencies,
            },
          });
        }
      } catch (err) {
        ctx.progress(`(todos) failed to mirror plan into todos list: ${(err as Error).message}`);
      }
    }
