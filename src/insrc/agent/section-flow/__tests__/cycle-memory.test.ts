...
/**
 * Phase alpha tests for plans/section-flow-fact-gap-loop.md.
 *
 * Ported (with path + module-location adjustments) from the deleted
 * `agent/tasks/code-analyzer/__tests__/cycle-memory.test.ts` (commit
 * c06a3cef579). Adds new tests for the section-flow-specific helpers
 * `gapFacts` + `isTrivialFastPath`.
 *
 * Covers:
 *   - summarizeCycleMemory(mem) -> prompt block string
 *   - computeCoverage(ledger, criteria, stepsById) -> coverage array
 *   - emptyCycleMemory(criteria) constructor
 *   - gapFacts(analysis) -> absent + partial subset
 *   - isTrivialFastPath(analysis) -> all-present predicate
 *
 * No LLM calls, no provider mocks; this is type-level + helper coverage
 * so future structural drift surfaces fast.
 */
...
function fact(id: string, status: RequiredFact['status'], extras: Partial<RequiredFact> = {}): RequiredFact {
	return {
		id,
		fact: `fact-${id}`,
		why:  `why-${id}`,
...
// ---------------------------------------------------------------------------
// gapFacts (section-flow-specific)
// ---------------------------------------------------------------------------

test('gapFacts: filters to absent + partial only', () => {
	const analysis: FactGapAnalysis = {
		requiredFacts: [
			fact('a', 'present'),
			fact('b', 'absent'),
			fact('c', 'partial'),
	...
	const gaps = gapFacts(analysis);
	assert.deepEqual(gaps.map(f => f.id), ['b', 'c', 'e']);
});

test('gapFacts: all-present analysis -> empty', () => {
	const analysis: FactGapAnalysis = {
		requiredFacts: [fact('a', 'present'), fact('b', 'present')],
		reasoning: 'r',
	};
	assert.deepEqual(gapFacts(analysis), []);
});

test('gapFacts: empty requiredFacts -> empty', () => {
	const analysis: FactGapAnalysis = { requiredFacts: [], reasoning: 'r' };
	assert.deepEqual(gapFacts(analysis), []);
});

// ---------------------------------------------------------------------------
// isTrivialFastPath (section-flow-specific)
// ---------------------------------------------------------------------------

test('isTrivialFastPath: all facts present + non-empty -> true', () => {
	const analysis: FactGapAnalysis = {
		requiredFacts: [fact('a', 'present'), fact('b', 'present')],
		reasoning: 'r',
	};
	assert.equal(isTrivialFastPath(analysis), true);
});

test('isTrivialFastPath: any absent or partial -> false', () => {
	const withAbsent: FactGapAnalysis = {
		requiredFacts: [fact('a', 'present'), fact('b', 'absent')],
		reasoning: 'r',
	};
	const withPartial: FactGapAnalysis = {
		requiredFacts: [fact('a', 'present'), fact('b', 'partial')],
		reasoning: 'r',
	};
	assert.equal(isTrivialFastPath(withAbsent), false);
	assert.equal(isTrivialFastPath(withPartial), false);
});

test('isTrivialFastPath: empty requiredFacts -> false (no facts means analyzer is unsure, not trivial)', () => {
	const analysis: FactGapAnalysis = { requiredFacts: [], reasoning: 'r' };
	assert.equal(isTrivialFastPath(analysis), false);
});
