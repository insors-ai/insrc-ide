...
function buildPlannerUser(
	...
	const lines: string[] = [
		...
		'',
		'## WIRING ORDER (strict, enforced by validator)',
		'A leaf may only wire `source=node` from nodes that appear EARLIER',
		'in execution order than itself. Specifically:',
		'  - The wired-from node must be an ancestor of this leaf, OR',
		'  - An earlier sibling in the SAME parent composition (i.e. it',
		'    appears BEFORE this leaf in the parent\'s `children` array).',
		'',
		'List child nodes in `children` in the order they will execute. If',
		'leaf B reads leaf A\'s output via `{source:"node", nodeId:"A"}`,',
		'then A MUST appear BEFORE B in the same `children` array (or be',
		'an ancestor of B). Forward references to later siblings, or to',
		'nodes in a different reviewable-root branch, are rejected.',
		'',
		'Worked pattern: a "discover -> analyze -> synthesize" 3-root',
		'composition can have `analyze` wire `{source:"node", nodeId:"discover", path:"$"}`',
		'because `discover` is an earlier sibling at the reviewable-root level.',
		'Inside `discover`, if leaf B wires from leaf A, A must be earlier',
		'in `discover.children` than B.',
		'',
		workedExampleHeader,
		WORKED_EXAMPLE_JSON,
		'',
